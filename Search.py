...
class Search(Tk):
    def __init__(self):
        ...
        self.canvas = Canvas(width=800, height=500, bg='brown')
        self.canvas.pack()
        self.iconbitmap(r'libico.ico')
        l1=Label(self,text="Search Library",bg='light blue', font=("Courier new",20,'bold')).place(x=290,y=20)
        l = Label(self, text="Search By",bg='light blue', font=("Courier new", 15, 'bold')).place(x=60, y=96)
        def insert(data):
        ...
        def ge():
            if (len(g.get())) == 0:
                ...
                try:
                    ...
                    messagebox.showerror("Error","Something goes wrong")
        b=Button(self,text="Find",width=15,bg='green',font=("Courier new",10,'bold'),command=ge).place(x=460,y=118)
        c=ttk.Combobox(self,textvariable=g,values=["Book Name","Author Name","Book Id"],width=40,state="readonly").place(x = 180, y = 100)
        en = Entry(self,textvariable=f,width=43).place(x=180,y=160)
        la = Label(self, text="Enter",bg='light blue', font=("Courier new", 15, 'bold')).place(x=100, y=150)
